...
On startup the registry seeds `nic.melendez`, `miguel.melendez`, and `example.melendez` (contacts, dual-stack in-bailiwick glue, apex A/AAAA, DS, `serverDeleteProhibited`). The TLD nameservers are dual-stack (`ns1`/`ns2` A and AAAA). Generate the DNS zone with DNSSEC enabled to sign those records.
